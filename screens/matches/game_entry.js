...
export default function Game_entry({route, navigation}) {
  ...
  submit_scores = () => {
    const add_scores = firebase.firestore().collection('matches').doc(itemID).collection('games').doc();
  ...
  return (
    <View style={{flex: 1}}>
        <Text style={styles.margin}>The max score is set at 30. There will be no conditioning rules set. Please make sure you put the scores in correctly. Especially in the UK, a lot of matches may have used weird scoring formats so this app tries to take that into account.</Text>
